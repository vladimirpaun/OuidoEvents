document.addEventListener('DOMContentLoaded', () => {
    ...
    const viewings = [
        { id: 1, client: 'Ioana Matei', venue: 'Villa Lac', date: formatDate(addDays(2)), hour: '11:00', host: 'Andreea', status: 'viewing_scheduled' },
        { id: 2, client: 'Căsătorim.ro', venue: 'Forest Lodge', date: formatDate(addDays(4)), hour: '09:30', host: 'Paul', status: 'viewing_request' },
        { id: 3, client: 'Eventify', venue: 'Urban Loft', date: formatDate(addDays(4)), hour: '15:00', host: 'Alex', status: 'viewing_rescheduled' },
    ...
    function renderBookingsTable() {
        ...
        filteredBookings.forEach(item => {
            ...
            nextStepCell.className = 'next-step-cell';
            const ownerMeta = statusMeta?.owner;
            if (ownerMeta) {
                nextStepCell.textContent = ownerMeta.nextStep || ownerMeta.actions?.[0] || '—';
                if (ownerMeta.actions && ownerMeta.actions.length) {
                    const hint = document.createElement('span');
                    hint.className = 'next-step-actions';
                    hint.textContent = `Acțiuni: ${ownerMeta.actions.join(' • ')}`;
                    nextStepCell.appendChild(hint);
                }
            } else {
                nextStepCell.textContent = '—';
            }
    ...
    function renderViewingsTable() {
        ...
        viewings
            ...
            .forEach(item => {
                ...
                statusCell.appendChild(createStatusChip(item.status, 'viewing'));
        ...
        switch (actionKey) {
            case 'confirm_availability':
                if (autoOfferEnabled) {
                    ...
                    showAutomationToast(`Locația ${booking.venue} a trimis automat oferta pentru ${booking.date}. Clientul vede „Ofertă primită”.`);
                } else {
            ...
            case 'reject':
                ...
                break;
            case 'add_note':
                window.alert('Adaugă o notă internă – funcționalitate în dezvoltare.');
                break;
            case 'open_details':
